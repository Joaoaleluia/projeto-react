...
app.post('/auth/register', async(req, res) => {
    ...
    if(!name || !email || !password){
        return res.status(400).json({ msg: 'Preeche todos os campos!'});
    }
...
app.post(' /auth/login', async(req, res) => {
    ...
    if(!email || !password){
        return res.status(400).json({ msg: 'Prencha email e senha!'});
    }
    ...
    const token = jwt.sign(
        ...
        process.env.JWT_SECRET || 'minhaChaveSuperSecrete',
        { expiresId: 'id'}
    );
